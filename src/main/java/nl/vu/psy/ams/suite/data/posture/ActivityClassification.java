...
public class ActivityClassification extends Thread {
    ...
    public static void generateClassificationFile() throws Exception {
        ...
        for (long i = 0; i < fL; i += nRead) {
            ...
            if (pressureAvailable) {
                ...
                int[] stepLocations = getStepsNuovo(dx, dy, dz);
    ...
    /**
     * method is used to detect steps in the given array using accleration
     * 
     * @param accelX in ms^2
     * @param accelY in ms^2
     * @param accelZ in ms^2
    ...
    public static int[] getStepsNuovo(double[] accelX, double[] accelY, double[] accelZ) {
        ...
        // Step detection tuning (unchanged)
        final double VECTOR_MAGNITUDE_THRESHOLD = 0.06; // m/s^2
        final double PHASE_RATE_THRESHOLD = 90.0; // deg/s
        final long MIN_STEP_INTERVAL_US = 400_000; // ms (hard minimum time gap between steps)
        ...
        // int maxSamplesBetween = (int) Math.round(1.20 * FS_DEC); // ~12 samples (soft
        // upper bound)
        ...
        for (int chunkStart = 0; chunkStart < r.length; chunkStart += CHUNK_SAMPLES) {
            ...
            for (int i = Math.max(chunkStart + 1, 1); i < Math.min(chunkEnd - 1, r.length - 1); i++) {
                ...
                if (i - lastStepIdxDec < minSamplesBetween)
                    continue;
                // If larger than maxSamplesBetween we allow it (slow cadence)
                ...
                long offsetSamples = origIdx + (long) (chunkno * size / 4);
                offsetSamples = correctForTicks(offsetSamples);
